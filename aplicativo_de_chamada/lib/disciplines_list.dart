...
class DisciplineList extends State<DisciplineListState> {
  ...
  Future<void> fetchDataFromAPI() async {
    final response =
        await http.post(Uri.parse('https://chamada-backend-sy8c.onrender.com/return_materias'));
  ...
  Future<void> checkSubcribedUserOnDiscipline(String idToSendForSubscription) async {
    final response =
      await http.post(Uri.parse('https://chamada-backend-sy8c.onrender.com/verificar_inscricao'), body: {'Email': widget.emailUser, 'ID_materia': idToSendForSubscription});
  ...
  Widget build(BuildContext context) {
    ...
    Visibility subscribeShowDialog(BuildContext context, String idToSendForSubscription) {
      ...
      return Visibility(
        ...
        onPressed: () async {
            final url = Uri.parse('https://chamada-backend-sy8c.onrender.com/Materia_Aluno');
